...
import { type GitStatus, getGitStatus } from "./git";
		...
		for (const project of projects) {
			...
			try {
				...
				if (isHealthy && project.status === "starting") {
					...
					console.log(
						`[Healthcheck] Project ${project.name} is failing... Marking as ERROR`,
					);
		...
		try {
			...
			for (const line of lines) {
				try {
					...
					const isRunning =
						data.State === "running" || data.Status?.includes("Up");
			...
			for (const p of projects) {
				...
				const isRunningInDocker =
					runningServicesMap.get(p.path)?.has(serviceName) || false;

				if (isRunningInDocker && p.status === "stopped") {
					console.log(
						`[Healthcheck] Docker Sync: Auto-detected ${p.name} as running`,
					);
					p.status = "running";
					updateProject(p);
					...
					console.log(
						`[Healthcheck] Docker Sync: Auto-detected ${p.name} as stopped`,
					);
					p.status = "stopped";
					updateProject(p);
